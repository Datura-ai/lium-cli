...
from ..utils import console, handle_errors, loading_status  # type: ignore
...
def show_executors(
    ...
    *,
    ...
    # Sort & limit
    executors = sorted(executors, key=_sort_key_factory(sort_by))
    if isinstance(limit, int) and limit > 0:
        executors = executors[:limit]
    ...
    for exe in executors:
        ...
        row = [
            f"[cyan]{_mid_ellipsize(getattr(exe, 'huid', '') or '')}[/]",
            _cfg(exe),
